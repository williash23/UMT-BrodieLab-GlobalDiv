...
### SPECIES LISTS PER *COUNTRY* INSTEAD OF PER GRID CELL ###
...
##### Manually add Myotis blythii  to species list for Maldives
# Maldives is country number 59
spp_int_ls[[59]] <- 
add <- as.data.frame("Myotis_blythii")
colnames(add) <- "species"
add$species <- as.character(add$species)
tmp <- spp_int_ls[[59]]  %>%
	rbind(add)
spp_int_ls[[59]]  <- tmp
...
out_mat <- matrix(NA, nrow = numcountries, ncol = 103)
start <- Sys.time()
start
...
for(i in 1:numcountries){
	...
	if(SR0 < 2){
		...
		SR_inb2 <- nrow(inb2_spp)
			...
			if(SR_nonnat2 == 0){PD_nonnat2 <- 0}else{
...
#write.csv(res_df, "FuncPhylo_mammals190710.csv", row.names=F) 
